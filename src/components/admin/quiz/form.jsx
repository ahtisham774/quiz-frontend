...
const Form = ({ subTitle, quiz, handleSubmit }) => {
    ...
    const handleImportQuestions = (event) => {
        ...
        if (file) {
            ...
            if (fileType === "xlsx") {
                // Handle .xlsx files (same as before)
                const reader = new FileReader();
                reader.onload = (e) => {
                    const data = new Uint8Array(e.target.result);
                    const workbook = XLSX.read(data, { type: "array" });
    ...
    const handleAddQuestion = () => {
        setFormData({ ...formData, questions: [...formData.questions, { question: "", img: "", hint: "", is_required: false, options: ["", "", "", ""], answer: "0" }] })
        setIsImported(true)
    }
    ...
    const onSubmit = (e) => {
        ...
        const questionsWithImageNames = formData.questions.map((question, index) => {
    ...
    return (
        <div className="w-full flex flex-col gap-5 p-4">
            ...
            <form onSubmit={onSubmit} className="flex flex-col gap-3 max-w-4xl w-full">
                <input
                    type="text"
                    id="name"
                            ...
                            <div key={index} className="flex flex-col gap-5">
                                ...
                                <div className="w-full flex flex-col gap-3">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            setFormData(prevData => {
                                                return {
                                                    ...prevData, questions: prevData.questions.map((q, i) => {
                                                        if (i === index) {
                                        ...
                                        <div className="relative w-full lg:w-1/2 h-[20rem] py-3">
                                            ...
                                            </button>
                                            {
                                                question.img &&
                                                    ...
                                                    <img src={`${question.img}`} alt="Uploaded Image" className="w-full h-full object-cover" />
                                                    // <img
                                    ...
                                    <div className="flex flex-col gap-3">
                                        {
                                            question?.options?.map((option, opIndex) => (
                                                <div key={opIndex} className="flex items-center w-full gap-2">
                                                    <input
                                                        type="text"
                                                        id="option"
                                ...
                                <div className="flex items-center gap-2 w-full justify-between">
                                    ...
                                    <label className="switch">
                                        <input type="checkbox" checked={question.is_required == "true"}
                                            onChange={(e) => {
                                                let newQuestions = [...formData.questions];
                ...
                <div className="flex items-center justify-between w-full flex-wrap gap-3">
                    <BtnOutline text="Add Question" css="bg-primary text-white outline-none" handleClick={handleAddQuestion} />
                    {!isImported && quiz == null && <div>
                        <input
                            type="file"
                            accept=".xlsx, .xls"
                            ...
                            id="fileInput"
                        />
                        <label htmlFor="fileInput" className="py-2 px-6 border-2 border-primary text-xl cursor-pointer font-bold bg-primary text-white outline-none ">
                            Import Questions
                        </label>
                    </div>}
                    {
